...
export class PerguntasComponent implements OnInit {
  ...
  selection = new SelectionModel<Pergunta>(true, []);
    ...
    merge(this.sort.sortChange, this.paginator.page)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.perguntaService.get(this.paginator.pageIndex + 1, this.paginator.pageSize, this.sort.active, this.sort.direction.toLocaleUpperCase(), this.search);
        }),
        map(data => {
  ...
  deletarData(item: Pergunta) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.perguntaService.delete(item).subscribe((res: any) => {
  ...
  deletarDatas() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.perguntaService.deleteSelected(this.selection.selected).subscribe((res: any) => {
  ...
  filter(e) {
    ...
    if (e) {
      ...
      this.filterSubscription = this.perguntaService.get(this.paginator.pageIndex + 1, this.paginator.pageSize, this.sort.active, this.sort.direction.toLocaleUpperCase(), e.toLocaleLowerCase())
        .subscribe((res: any) => {
          this.data = this.paginator.pageIndex == 0 ? res.perguntas : this.data.concat(res.perguntas);
        });
    } else {
      this.buscar();
    }
