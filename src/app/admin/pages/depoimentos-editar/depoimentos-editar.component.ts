...
@Component({
  selector: 'app-depoimentos-editar',
  templateUrl: './depoimentos-editar.component.html',
  styleUrls: ['./depoimentos-editar.component.scss']
})
export class DepoimentosEditarComponent implements OnInit {
  ...
  buscar(id: number) {
    this.depoimentoService.getById(id).subscribe((res: Depoimento) => this.data = res);
  }
  ...
  submit(f: NgForm) {
    ...
    this.depoimentoService.patch(this.data)
      .subscribe((res: any) => {
        this.submitImages(this.file, '/depoimento/imagem?id=' + this.data.id);
