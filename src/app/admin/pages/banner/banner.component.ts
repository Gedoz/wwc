...
export class BannerComponent implements OnInit {
  ...
  ngOnInit() {
    this.bannerService.get().subscribe((res: Banner) => this.data = res);
  }
  ...
  submit(f: NgForm) {
    ...
    this.bannerService.patch(this.data)
      .subscribe((res: any) => {
        this.submitImages(this.file, '/banner/imagem?id=' + this.data.id);
  ...
  onFileSelected(files) {
    ...
    this.submitImages(this.file, '/banner/imagem?id=-99').then((res: any) => {
  ...
  submitImages(file: File, url: string) {
    ...
    return new Promise((resolve, reject) => {
      this.bannerService.postFile(file, url, 'image')
