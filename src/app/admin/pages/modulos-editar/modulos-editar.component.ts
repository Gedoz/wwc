...
@Component({
  ...
  styleUrls: ['./modulos-editar.component.scss']
})
export class ModulosEditarComponent implements OnInit {
  ...
  buscar(id: number) {
    this.moduloService.getById(id).subscribe(res => this.data = res);
  }
  ...
  submit(f: NgForm) {
    ...
    this.moduloService.patch(this.data)
      .subscribe((res: any) => {
        this.submitImages(this.file, '/modulo/imagem?id=' + this.data.id);
